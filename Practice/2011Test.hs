...
tryToLookUp x b dict
  = fromMaybe b (lookup x dict)

-- Pre: The given value is in the table
reverseLookUp :: Eq b => b -> [(a, b)] -> [a]
reverseLookUp x []
  = []
reverseLookUp x ((k, v) : ps)
  | x == v    = k : reverseLookUp x ps
  | otherwise = reverseLookUp x ps
...
inferType (Prim x) env
  = lookUp x primTypes
inferType (Cond _ then' else') env
  | thenT == elseT  = thenT
  | otherwise       = TErr
  where
    thenT = inferType then' env
    elseT = inferType else' env
inferType (App func arg) env
  = inferApp (inferType func env) (inferType arg env)
  where
    inferApp (TFun t t') argType
      | argType == t  = t'
      | otherwise     = TErr
    inferApp _ _
      = TErr
